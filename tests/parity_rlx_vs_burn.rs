...
fn locate_paths() -> Option<(PathBuf, PathBuf, PathBuf)> {
    let weights = std::env::var("ZUNA_WEIGHTS")
        .ok()
        .map(PathBuf::from)
        .or_else(|| {
    ...
    let config = std::env::var("ZUNA_CONFIG")
        .ok()
        .map(PathBuf::from)
        .or_else(|| {
...
fn rlx_encoder_matches_burn_encoder() {
    ...
    let pre = preprocess_fif_cpu(&fif, &data_cfg, 10.0).expect("preprocess_fif_cpu");
    assert!(!pre.epochs.is_empty(), "expected at least one epoch");
    let ep = &pre.epochs[0];
    eprintln!(
        "→ epoch 0: s={} tf={} n_channels={}",
        ep.s, ep.tf, ep.n_channels
    );
    ...
    let burn_batches = burn_enc
        .preprocess_fif(&fif, 10.0)
        .expect("burn preprocess_fif")
        .0;
    let burn_embeddings = burn_enc
        .encode_batches(burn_batches)
    ...
    eprintln!(
        "→ burn[0]: shape={:?} first8={:?}",
        burn_first.shape,
        &burn_first.embeddings[..8.min(burn_first.embeddings.len())]
    );
    ...
    eprintln!(
        "→ rlx[0]:  shape={:?} first8={:?}",
        rlx_emb.shape,
        &rlx_emb.embeddings[..8.min(rlx_emb.embeddings.len())]
    );

    // 4. Compare.
    assert_eq!(
        burn_first.shape, rlx_emb.shape,
