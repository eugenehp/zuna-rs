...
use crate::model::{linear_zeros, EncoderDims};
...
impl<B: Backend> EncoderTransformer<B> {
    pub fn new(d: EncoderDims, device: &B::Device) -> Self {
        let layers = (0..d.n_layers)
            .map(|_| EncoderBlock::new(d.block, device))
            .collect();
        Self {
            tok_embeddings: linear_zeros(d.input_dim, d.block.dim, true, device),
            registers: Param::initialized(ParamId::new(), Tensor::zeros([1, d.input_dim], device)),
            layers,
            norm: RMSNorm::new(d.block.dim, d.block.norm_eps, device),
            output: linear_zeros(d.block.dim, d.output_dim, false, device),
    ...
    pub fn forward(
        ...
        let interleaved =
            Tensor::stack::<4>(vec![regs, token_values], 2).reshape([b, s * (df + 1), d]);
...
fn repeat_interleave_rows<B: Backend>(t: Tensor<B, 2, Int>, repeats: usize) -> Tensor<B, 2, Int> {
    let [s, c] = t.dims();
