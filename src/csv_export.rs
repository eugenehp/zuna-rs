...
pub fn fif_to_csv(
    ...
    let raw = open_raw(fif_path).with_context(|| format!("opening FIF: {}", fif_path.display()))?;
    ...
    let written_names: Vec<String> = keep_indices.iter().map(|&i| all_names[i].clone()).collect();
    ...
    writeln!(
        file,
        "# sfreq={sfreq} n_channels={} n_samples={n_times}",
        written_names.len()
    )?;
