...
/// Precomputed per-position RoPE tables for one decoder step.
/// Each buffer is `[1, S, 1, head_dim/2]` row-major.
...
impl ZunaDecoder {
    pub fn load(
        ...
        let (params, n_heads, arch) = build_decoder_params(&mut raw, &model_cfg)?;
    ...
    pub fn describe(&self) -> String {
        ...
        format!(
            "{} decoder (RLX, dev={:?})  dim={}  layers={}  head_dim={}  t_dim={}  σ={}",
            self.arch.label(),
            self.device,
            c.dim,
            c.n_layers,
    ...
    pub fn decode_from_embedding(
        ...
        anyhow::ensure!(
            ep.shape[1] == ed,
            "embedding output_dim mismatch: expected {ed}, got {}",
            ep.shape[1]
        );
        ...
        let (cos, sin) =
            rdata::precompute_rope(&ep.tok_idx, &self.rope_table, head_dim, rope_dim, s);
        ...
        for i in (1..=steps).rev() {
            ...
            let vc = self.forward_step(&z, &ep.embeddings, t_val, rope, b, s)?;
            let vc = if (cfg - 1.0).abs() > 1e-4 {
                let enc_zero = vec![0f32; b * s * ed];
                let vc_u = self.forward_step(&z, &enc_zero, t_val, rope, b, s)?;
                vc_u.iter()
                    .zip(vc.iter())
                    .map(|(u, c)| u + cfg * (c - u))
                    .collect()
