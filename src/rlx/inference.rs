...
impl ZunaInference {
    ...
    pub fn load(
        ...
        Ok((Self { encoder, decoder, data_cfg: DataConfig::default() }, ms))
    }
    ...
    pub fn run_epoch(
        ...
        let embedding = self.encoder.encode_one(
            &ep.eeg_tokens, &ep.tok_idx, &ep.chan_pos,
            ep.n_channels, ep.tc,
        ).context("encoder.encode_one")?;
        self.decoder.decode_from_embedding(&embedding, steps, cfg, data_norm, noise_seed)
    }
