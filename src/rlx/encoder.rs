...
impl ZunaEncoder {
    ...
    pub fn load(
        ...
        let (mut params, n_heads, arch) = build_encoder_params(&mut raw, &model_cfg)?;
    ...
    pub fn describe(&self) -> String {
        ...
        format!(
            "{} encoder (RLX, dev={:?})  dim={}  layers={}  head_dim={}  out_dim={}",
            self.arch.label(),
            self.device,
            c.dim,
            c.n_layers,
    ...
    pub fn encode_one(
        ...
        let x = rdata::preinterleave(
            ...
            b,
            s,
            self.model_cfg.input_dim,
            df,
        );
        let tok_idx_x = rdata::repeat_token_idx(tok_idx, s, df);
        let (cos, sin) =
            rdata::precompute_rope(&tok_idx_x, &self.rope_table, head_dim, rope_dim, s2);
